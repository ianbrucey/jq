                    ...
                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                ...
                                <span class="ms-2 text-sm text-base-content/60 dark:text-base-content/60">{{ $permission }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="space-y-6">
                        @foreach ($this->user->tokens->sortBy('name') as $token)
                            <div class="flex items-center justify-between">
                                <div class="break-all dark:text-base-100">
                                    {{ $token->name }}
                                </div>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        ...
        <x-slot name="content">
            ...
            <x-input x-ref="plaintextToken" type="text" readonly :value="$plainTextToken"
                class="mt-4 bg-base-200 px-4 py-2 rounded font-mono text-sm text-base-content/50 w-full break-all"
